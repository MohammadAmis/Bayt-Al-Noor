...
class ChatPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final state = ref.watch(chatViewModelProvider(
        chatId ?? 'draft_${initialProfile?.id}',
        initialProfile: initialProfile));
    final notifier = ref.read(chatViewModelProvider(
            chatId ?? 'draft_${initialProfile?.id}',
            initialProfile: initialProfile)
        .notifier);
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onTap: (chatId == null || chatId!.startsWith('draft_'))
              ? null
              : () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => ChatInfoPage(
                          chatId: chatId!, chatName: state.chatName),
                    ),
                  ),
          child: Row(
            children: [
              CircleAvatar(
                backgroundImage:
                    chatAvatar != null ? NetworkImage(chatAvatar!) : null,
      ...
      body: Column(
        children: [
          ...
          MessageInputBar(
            onSend: (text, isImage) {
              if (isImage) {
                notifier.sendImageMessage(text);
              } else {
